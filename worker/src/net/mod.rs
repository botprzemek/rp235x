...
impl Net {
    pub async fn init_trng(peripherals: TrngPeripherals) -> [u8; 8] {
        ...
        let mut trng = trng::Trng::new(peripherals.trng, shared::interrupts::Irqs, trng_config);
        let mut seed = [0u8; 8];
    ...
    pub async fn init_cyw43(
        ...
        let mut pio = Pio::new(peripherals.pio, shared::interrupts::Irqs);
        let dma_channel = dma::Channel::new(peripherals.dma, shared::interrupts::Irqs);
        let spi = PioSpi::new(
            &mut pio.common,
            pio.sm0,
    ...
    pub async fn init_wifi(control: &mut Control<'static>) {
        let config = crate::shared::config::read();
...
//     let sender = shared::channels::CLOCK_CHANNEL.sender();
//     let ntp_host = {
...
//         for _ in 0..3600 {
//             let update = shared::channels::ClockUpdate {
